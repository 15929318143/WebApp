...
async def create_pool(loop, user, password, db, **kw):
    ...
    __pool = await aiomysql.create_pool(
            ...
            user = user,                            #通过关键字参数传入user
            password = password,                    #通过关键字参数传入密码
            db = db,                                #数据库名字
            host = kw.get('host','localhost'),      #默认定义host名字为localhost
            port = kw.get('port',3306),             #默认定义mysql的端口是3306
            charset = kw.get('charset','utf-8'),    #默认数据库字符集是utf8
...
async def execute(sql, args, autocommit=True):
    ...
    async with __pool.get() as conn:
        ...
        try:
            ...
            if not autocommit:
                # 连接对象的db.commit()方法表示事务提交
                await conn.commit()
        # 如果你确实要捕获所有异常，那么你就会得到BaseException
        except BaseException as e:
            if not autocommit:
                # 连接对象的db.rollback()方法表示事务回退
                await conn.rollback()
            raise e
        return affected
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def countRows(cls, where=None, args=None):
        ...
        sql = ['select count(*) _num_from `%s`' % (cls.__table__)]
        if where:
            sql.append('where %s' % (where))
    ...
    async def find(cls, pk):
        ...
        resultset = await select('%s where `%s` = ?' % (cls.__select__,cls__primary_key__),[pk], 1)
